export function StatusBadge({ value }: { value: string }) {
  ...
  if (key === 'OK' || key === 'AKTIF' || key === 'APPROVED' || key === 'ONAYLANDI') {
  ...
  if (key === 'PENDING' || key === 'BEKLIYOR' || key === 'BEKLEMEDE') {
    return <span className="status-badge status-badge-pending">{value}</span>
  }
  if (key === 'REJECTED' || key === 'PASIF' || key === 'REDDEDILDI') {

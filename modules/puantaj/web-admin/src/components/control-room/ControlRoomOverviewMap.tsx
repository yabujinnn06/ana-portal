...
export function ControlRoomOverviewMap({
  ...
  useEffect(() => {
    ...
    // Live polling refreshes `points`/`clusteredPoints` identity every cycle even when
    // nothing actually moved. Skip the clearLayers()+rebuild when the rendered set
    // (position/state per marker or cluster membership) and the selection are identical
    // to the previous run, so identical poll results don't cause marker flicker.
    const renderSignature = clusteredPoints
      .map((item) =>
        item.kind === 'single'
          ? `s:${item.point.employeeId}:${item.point.lat.toFixed(6)}:${item.point.lon.toFixed(6)}:${item.point.tsUtc}:${item.point.locationState}:${item.point.riskStatus}`
          : `c:${[...item.pointIds].sort((a, b) => a - b).join(',')}:${item.centerLat.toFixed(6)}:${item.centerLon.toFixed(6)}:${item.hasCritical}`,
      )
      .join('|')
    const signatureKey = `${renderSignature}::sel:${selectedEmployeeId ?? 'none'}`

    if (signatureKey === lastRenderSignatureRef.current) {
      return
    }
    lastRenderSignatureRef.current = signatureKey
    ...
    clusteredPoints.forEach((item) => {
      if (item.kind === 'single') {
        const tooltipHtml = [
          `<strong>${item.point.employeeName}</strong>`,
          item.point.departmentName ? ` · ${item.point.departmentName}` : '',
          `<br/>${todayStatusLabel(item.point.todayStatus)} · ${controlRoomRiskLabel(item.point.riskStatus)}`,
          `<br/>${controlRoomLocationLabel(item.point.locationState)} · ${formatRelative(item.point.tsUtc)}`,
        ].join('')
        const marker = L.marker([item.point.lat, item.point.lon], {
          ...
          .bindTooltip(tooltipHtml, { direction: 'top', offset: [0, -10], opacity: 0.95 })
          .addTo(layer)

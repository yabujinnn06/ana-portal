...
const ACTION_OPTIONS: { value: string; label: string }[] = [
  ...
  { value: 'EMPLOYEE_APP_LOCATION_PING', label: 'Employee konum pingi' },
]
...
const EVENT_OPTIONS: { value: string; label: string }[] = [
  ...
  { value: 'app_last_seen', label: 'Uygulama son görülme' },
]
...
const ACTION_LABELS: Record<string, string> = {
  ...
  EMPLOYEE_APP_LOCATION_PING: 'Employee konum pingi',
}
...
export function SystemLogsPage() {
  ...
  // Audit logs arrive newest-first, but the terminal appends rows top-to-bottom and
  // autoscrolls to the bottom in follow mode — render a chronological (oldest-first)
  // copy so the newest entry actually lands in view at the bottom.
  const terminalLogs = useMemo(() => [...visibleLogs].reverse(), [visibleLogs])

  const selectedLogLocation = useMemo(() => {
    if (!selectedLog) {
      return null
    }
    const lat = toFiniteNumber(selectedLog.details?.lat)
    const lon = toFiniteNumber(selectedLog.details?.lon)
    if (lat === null || lon === null) {
      return null
    }
    const accuracyM = toFiniteNumber(selectedLog.details?.accuracy_m)
    const batteryLevel = toFiniteNumber(selectedLog.details?.battery_level)
    const networkTypeRaw = selectedLog.details?.network_type
    const networkType = typeof networkTypeRaw === 'string' && networkTypeRaw.trim() ? networkTypeRaw : null
    return { lat, lon, accuracyM, batteryLevel, networkType }
  }, [selectedLog])
            ...
            <aside className="rounded-xl border border-slate-200 bg-slate-50 p-3">
              ...
              {selectedLog === null ? (
                ...
                <div className="mt-3 space-y-3">
                  ...
                  {selectedLogLocation ? (
                    <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-2 text-sm text-emerald-900">
                      <p className="text-xs font-semibold uppercase tracking-wide text-emerald-700">Konum</p>
                      <p className="mt-1 font-mono text-xs">
                        {selectedLogLocation.lat.toFixed(5)}, {selectedLogLocation.lon.toFixed(5)}
                      </p>
                      {selectedLogLocation.accuracyM !== null ? (
                        <p className="mt-1 text-xs text-emerald-800">Doğruluk: ~{selectedLogLocation.accuracyM.toFixed(0)} m</p>
                      ) : null}
                      {selectedLogLocation.batteryLevel !== null ? (
                        <p className="text-xs text-emerald-800">Batarya: %{selectedLogLocation.batteryLevel}</p>
                      ) : null}
                      {selectedLogLocation.networkType ? (
                        <p className="text-xs text-emerald-800">Ağ: {selectedLogLocation.networkType}</p>
                      ) : null}
                      <a
                        href={`https://www.google.com/maps?q=${selectedLogLocation.lat},${selectedLogLocation.lon}`}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="mt-2 inline-block text-xs font-semibold text-emerald-700 underline hover:text-emerald-900"
                      >
                        Haritada aç
                      </a>
                    </div>

...
function formatRelative(value: string | null | undefined): string {
  ...
  if (diffSec < 86400) {
    return `${Math.floor(diffSec / 3600)} saat once`
  }
...
export function AdminUsersPage() {
  ...
  const activityQuery = useQuery({
    queryKey: ['admin-user-activity', detailUser?.username],
    queryFn: () => getAuditLogs({ actor_id: detailUser!.username, limit: 20 }),
    enabled: Boolean(detailUser),
  })
              ...
              <>
                ...
                <section>
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <h5 className="text-sm font-semibold text-slate-900">Son İşlemler</h5>
                    {detailUser?.is_online ? (
                      ...
                      <span className="text-xs text-slate-500">
                        Son görülme: {formatRelative(detailUser?.last_seen_at)}
                      </span>
                    )}
                  </div>
                  <div className="mt-2 grid gap-2 md:grid-cols-2">
                    <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm">
                      Son görülme: <strong>{formatDateTime(detailUser?.last_seen_at)}</strong>
                    </div>
                    <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm">
                      Son giriş: <strong>{formatDateTime(detailUser?.last_login_at)}</strong>
                    </div>
                  </div>
                  <div className="mt-2 max-h-60 overflow-auto rounded-lg border border-slate-200">
                    <table className="min-w-full text-left text-xs">
                      ...
                      <tbody>
                        {(activityQuery.data?.items ?? []).map((log) => (
                          <tr key={log.id} className="border-t border-slate-100">
                            <td className="px-2 py-2 whitespace-nowrap">{formatDateTime(log.ts_utc)}</td>
                            ...
                            <td className="px-2 py-2 font-mono text-[11px]">{log.ip ?? '-'}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                    {activityQuery.isLoading ? (
                      <p className="px-2 py-2 text-sm text-slate-500">Yükleniyor...</p>
                    ) : (activityQuery.data?.items ?? []).length === 0 ? (
                      <p className="px-2 py-2 text-sm text-slate-500">Bu admin için işlem kaydı bulunamadı.</p>
                    ) : null}
                  </div>
                </section>
              </>

...
import { ScanErrorOverlay, ScanSuccessOverlay } from '../features/home/ScanFeedbackOverlays'
import {
  CheckoutConfirmModal,
  DemoConfirmModal,
  DemoLocationPromptModal,
} from '../features/home/ConfirmModals'
import type { LastAction } from '../features/home/types'
import { useScanFeedback } from '../hooks/useScanFeedback'
import { useEmployeeStatus, type TodayStatus } from '../hooks/useEmployeeStatus'
...
// LastAction tipini features/home/types'a tasidik; lokal interface kaldirildi.
import { playCheckoutPromptTone, playDemoPromptTone } from '../utils/audio'
import { clearStoredDeviceFingerprint, getStoredDeviceFingerprint } from '../utils/device'
import { getCurrentLocation } from '../utils/location'

// TodayStatus tipi hooks/useEmployeeStatus'a tasindi, oradan import edilir.
...
// LastAction interface'i features/home/types.ts'e tasindi.
...
// eventTypeLabel utils/attendance.ts'e tasindi.
...
// 5 sub-component (EmployeeFocusModal, EmployeeHeaderSection, MainActionCard,
// LastActionSummarySection, CriticalAlertsSection) + SecondaryDisclosure
// features/home/HomeSections.tsx'e tasindi. Import yukarida.
...
export function HomePage() {
  ...
  const {
    successOpen: scanSuccessFxOpen,
    errorPayload: scanErrorFx,
    triggerSuccess: triggerScanSuccessFx,
    triggerError: triggerScanErrorFx,
    dismissError: dismissScanErrorFx,
  } = scanFeedback
  ...
  // todayStatus + statusSnapshot useEmployeeStatus hook'unda. Hook cagrisi
  // handleDeviceNotClaimed tanimlandiktan sonra (asagida).
  // demoHistory state'leri useEmployeeDemoHistory hook'una tasindi (asagida).
  // leaveHistory state'leri useEmployeeLeaves hook'una tasindi.
  ...
  const handleDeviceNotClaimed = useCallback((parsed: ParsedApiError): boolean => {
    if (parsed.code !== 'DEVICE_NOT_CLAIMED') {
      ...
      // demoHistory + isDemoHistoryReady useEmployeeDemoHistory hook
      // deviceFingerprint=null gorunce kendi resetlerini yapar.
  ...
  // Scan feedback (success/error overlay + ses + titresim) hook'a tasindi:
  // useScanFeedback. Destructure yukarida.

  // Employee status hook'u: today_status + statusSnapshot + setters.
  // handleDeviceNotClaimed ref-baglı, dependency tetiklemez.
  const { todayStatus, statusSnapshot, setTodayStatus, setStatusSnapshot } = useEmployeeStatus({
    deviceFingerprint,
    refreshToken: lastAction?.response.event_id,
  ...
  const {
    demoHistory,
    isLoading: isDemoHistoryLoading,
    isReady: isDemoHistoryReady,
  } = useEmployeeDemoHistory({
    deviceFingerprint,
    enabled: Boolean(deviceFingerprint) && isHistoryDisclosureOpen,
    lastStartedAtUtc: statusSnapshot?.last_demo_started_at_utc ?? null,
    lastEndedAtUtc: statusSnapshot?.last_demo_ended_at_utc ?? null,
    onAuthFailure: handleDeviceNotClaimed,
  })
  ...
  const {
    ...
    isReady: isLeaveHistoryReady,
  } = useEmployeeLeaves({
    deviceFingerprint,
    enabled: shouldLoadLeaveHistory,
  ...
  // Tum hook'lar deviceFingerprint=null oldugunda kendi reset'lerini yapar.

  // Leave history fetch useEmployeeLeaves hook'unda.
  ...
  const runQrScan = async (rawQrValue: string) => {
    ...
    try {
      ...
      if (parsed.code === 'SECOND_CHECKIN_APPROVAL_REQUIRED') {
        ...
        const errorText = sanitizeEmployeeActionMessage(
          parsed,
          'QR işlemi için gerekli cihaz hazırlığı tamamlanamadı. Ayarları kontrol edip tekrar deneyin.',
        )
        triggerScanErrorFx({
          title: 'QR onaylanmadi',
          message: errorText,
  ...
  return (
    <main className="phone-shell employee-shell">
      <div className="employee-layout">
        ...
        <ScanSuccessOverlay open={scanSuccessFxOpen} />
        <ScanErrorOverlay payload={scanErrorFx} onDismiss={dismissScanErrorFx} />
        ...
        <CheckoutConfirmModal
          ...
          onConfirm={() => { setIsCheckoutConfirmOpen(false); void runCheckout() }}
        />
        ...
        <DemoConfirmModal
          ...
          onConfirm={() => { setIsDemoConfirmOpen(false); void runDemoMark() }}
        />

        <DemoLocationPromptModal
          isOpen={isDemoLocationPromptOpen}
          onClose={() => setIsDemoLocationPromptOpen(false)}
          onRetry={() => { setIsDemoLocationPromptOpen(false); void runDemoMark() }}
        />

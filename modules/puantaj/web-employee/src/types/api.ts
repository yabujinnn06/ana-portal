...
export interface BreakStatusRecord {
  employee_id: number
  on_break: boolean
...
export interface EmployeeStatusResponse {
  ...
  extra_checkin_approval?: 'NONE' | 'PENDING' | 'APPROVED'
}
...
export interface EmployeeAppPresencePingRequest {
  device_fingerprint: string
  source?: 'APP_OPEN' | 'APP_CLOSE' | 'LOCATION_PING' | 'DEMO_START' | 'DEMO_END' | 'DEMO_MARK'
  lat?: number
  lon?: number
  accuracy_m?: number | null
  provider?: string
...
export interface DepoOzetResponse {
  izin: boolean

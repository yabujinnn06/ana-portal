...
from sqlalchemy import and_, case, delete as sa_delete, func, or_, select, update as sa_update
...
from app.schemas import (
    ...
    AdminPresenceRosterEntry,
    DailyBoardResponse,
    EmployeeAttendanceHistoryResponse,
    ControlRoomSummaryRead,
    SchedulePlanRead,
    SchedulePlanUpsertRequest,
    SaturdayRotationRead,
    SaturdayRotationUpsertRequest,
    DepartmentWeeklyRuleRead,
    DepartmentWeeklyRuleUpsert,
    EmployeeWeeklyRestDayRead,
    EmployeeWeeklyRestDayUpsert,
    SpecialDayEmployeeOverrideRead,
    SpecialDayEmployeeOverrideUpsertRequest,
...
from app.services.exports import (
    build_daily_board_xlsx_bytes,
    build_device_invites_xlsx_bytes,
    build_leave_ledger_xlsx_bytes,
...
from app.services.attendance_board import (
    build_daily_attendance_board,
    build_employee_attendance_history,
)
from app.services.presence import (
    ONLINE_WINDOW_SECONDS,
    list_active as list_active_admins,
    touch_presence,
)
...
def _to_admin_user_read(
    ...
    *,
    ...
    last_seen = admin_user.last_seen_at
    is_online = (
...
def _admin_me_response_from_claims(claims: dict[str, Any]) -> AdminMeResponse:
...
def admin_login(
    ...
    if should_allow_refresh():
        ...
        try:
            ...
            db.rollback()
            logger.exception(
                "admin_refresh_token_persist_failed",
                extra={
                    "request_id": request_id,
                    "admin_user_id": identity.get("admin_user_id"),
    ...
    persist_admin_last_seen(db, identity.get("admin_user_id"), login=True)
    ...
    return AdminAuthResponse(
        ...
        refresh_token=refresh_token,
        user=_admin_me_response_from_claims(access_claims),
    )
...
def admin_me(
    ...
    # Admin access tokens can live for a long time; always resolve the
    # *current* DB permissions/role here so the panel reflects permission
    # changes without requiring the admin to log out and back in.
    admin_user_id = claims.get("admin_user_id")
    if admin_user_id:
        admin_user = db.get(AdminUser, admin_user_id)
        if admin_user is None or not admin_user.is_active:
            raise ApiError(status_code=401, code="INVALID_TOKEN", message="Admin account is no longer active.")
        claims = {
            **claims,
            "is_super_admin": bool(admin_user.is_super_admin),
            "permissions": admin_user.permissions,
        }
    return _admin_me_response_from_claims(claims)
...
def list_employees(
    ...
    region_id: int | None = Query(default=None, ge=1),
    device_status: Literal["any", "none", "no_active"] | None = Query(default=None),
    db: Session = Depends(get_db),
) -> list[EmployeeRead]:
    device_counts_subq = (
        select(
    ...
    stmt = (
        select(
            ...
            func.coalesce(device_counts_subq.c.active_device_count, 0).label("active_device_count"),
        )
        .options(selectinload(Employee.region))
        .outerjoin(device_counts_subq, device_counts_subq.c.employee_id == Employee.id)
    ...
    if device_status == "none":
        stmt = stmt.where(func.coalesce(device_counts_subq.c.device_count, 0) == 0)
    elif device_status == "no_active":
        stmt = stmt.where(func.coalesce(device_counts_subq.c.active_device_count, 0) == 0)

    rows = db.execute(stmt).all()
    results: list[EmployeeRead] = []
    for employee, device_count, active_device_count in rows:
        employee_read = _to_employee_read(employee)
        employee_read.device_count = int(device_count)
        employee_read.active_device_count = int(active_device_count)
        results.append(employee_read)
    return results
...
def get_attendance_daily_board(
    ...
    return build_daily_attendance_board(
        db,
...
@router.get(
    "/api/admin/attendance/daily-board/employees/{employee_id}",
    response_model=EmployeeAttendanceHistoryResponse,
    dependencies=[Depends(require_admin_permission("attendance_events"))],
)
def get_attendance_daily_board_employee_history(
    employee_id: int,
    ...
    if employee is None:
        raise HTTPException(status_code=404, detail="Employee not found")
    return build_employee_attendance_history(
        db,
        employee_id=employee_id,
        start_date=start_date,
        end_date=end_date,
...
def _admin_presence_response(db: Session) -> AdminPresenceResponse:
    now = datetime.now(timezone.utc)
    live = list_active_admins()
    online_entries = [AdminPresenceEntry(**entry) for entry in live]

    live_by_username: dict[str, dict[str, Any]] = {}
    for entry in live:
        uname = str(entry.get("username") or "").strip().casefold()
        if not uname:
            continue
        current = live_by_username.get(uname)
        if current is None or entry["last_seen_utc"] > current["last_seen_utc"]:
            live_by_username[uname] = entry

    roster: list[AdminPresenceRosterEntry] = []
    seen: set[str] = set()
    rows = db.scalars(
        select(AdminUser).where(AdminUser.is_active.is_(True)).order_by(AdminUser.username)
    ).all()
    for admin_user in rows:
        key = admin_user.username.strip().casefold()
        seen.add(key)
        live_entry = live_by_username.get(key)
        last_seen = admin_user.last_seen_at
        if last_seen is not None and last_seen.tzinfo is None:
            last_seen = last_seen.replace(tzinfo=timezone.utc)
        if live_entry is not None and (last_seen is None or live_entry["last_seen_utc"] > last_seen):
            last_seen = live_entry["last_seen_utc"]
        is_online = live_entry is not None or (
            last_seen is not None and (now - last_seen).total_seconds() <= ONLINE_WINDOW_SECONDS
        )
        roster.append(
            AdminPresenceRosterEntry(
                username=admin_user.username,
                full_name=admin_user.full_name,
                role="Super Admin" if admin_user.is_super_admin else "Admin",
                is_online=is_online,
                last_seen_utc=last_seen,
                last_login_utc=admin_user.last_login_at,
            )
        )

    # DB'de satiri olmayan canli oturumlar (env super admin gibi)
    for key, entry in live_by_username.items():
        if key in seen:
            continue
        roster.append(
            AdminPresenceRosterEntry(
    ...
    roster.sort(
        key=lambda item: (
            not item.is_online,
            -(item.last_seen_utc.timestamp() if item.last_seen_utc else 0.0),
        )
    )
    return AdminPresenceResponse(generated_at_utc=now, online=online_entries, roster=roster)


def persist_admin_last_seen(db: Session, admin_user_id: Any, *, login: bool = False) -> None:
    """admin_users.last_seen_at (login=True ise last_login_at) kalici guncelle. Heartbeat'i kismarak yazar."""
    try:
    ...
    stmt = sa_update(AdminUser).where(AdminUser.id == aid)
    if login:
        values["last_login_at"] = now
    else:
        threshold = now - timedelta(seconds=50)
        stmt = stmt.where(or_(AdminUser.last_seen_at.is_(None), AdminUser.last_seen_at < threshold))
    try:
...
@router.post("/api/admin/presence/heartbeat", response_model=AdminPresenceResponse)
def admin_presence_heartbeat(
    claims: dict[str, Any] = Depends(require_admin),
    db: Session = Depends(get_db),
) -> AdminPresenceResponse:
    key = str(claims.get("admin_user_id") or claims.get("username") or claims.get("sub") or "admin")
    touch_presence(
        key=key,
        username=str(claims.get("username") or claims.get("sub") or "admin"),
        full_name=claims.get("full_name"),
        role=claims.get("role"),
    )
    persist_admin_last_seen(db, claims.get("admin_user_id"))
    return _admin_presence_response(db)


@router.get("/api/admin/presence", response_model=AdminPresenceResponse)
def admin_presence(
...
def upsert_work_rule(payload: WorkRuleUpsert, db: Session = Depends(get_db)) -> WorkRuleRead:
    ...
    if rule is None:
        ...
        rule.overtime_threshold_minutes = payload.overtime_threshold_minutes
...
@router.post(
    "/admin/employee-weekly-rest-days",
    response_model=EmployeeWeeklyRestDayRead,
    dependencies=[Depends(require_admin_permission("schedule", write=True))],
)
def upsert_employee_weekly_rest_day(
    payload: EmployeeWeeklyRestDayUpsert,
    request: Request,
    db: Session = Depends(get_db),
) -> EmployeeWeeklyRestDayRead:
    employee = db.get(Employee, payload.employee_id)
    ...
    rest_day = db.scalar(
        select(EmployeeWeeklyRestDay).where(
            EmployeeWeeklyRestDay.employee_id == payload.employee_id,
            EmployeeWeeklyRestDay.weekday == payload.weekday,
        )
    )
    if rest_day is None:
        ...
        rest_day.is_active = payload.is_active
        rest_day.note = payload.note
    ...
    log_audit(
        ...
        actor_type=AuditActorType.ADMIN,
        actor_id="admin",
        action="EMPLOYEE_WEEKLY_REST_DAY_UPSERT",
        success=True,
        entity_type="employee_weekly_rest_day",
        entity_id=str(rest_day.id),
        ip=_client_ip(request),
        user_agent=_user_agent(request),
        details={
            "employee_id": rest_day.employee_id,
            "weekday": rest_day.weekday,
            "is_active": rest_day.is_active,
        },
        request_id=getattr(request.state, "request_id", None),
    )
    return rest_day
...
@router.get(
    "/admin/employee-weekly-rest-days",
    response_model=list[EmployeeWeeklyRestDayRead],
    dependencies=[Depends(require_admin_permission("schedule"))],
)
def list_employee_weekly_rest_days(
    employee_id: int | None = Query(default=None, ge=1),
    active_only: bool = Query(default=True),
    db: Session = Depends(get_db),
) -> list[EmployeeWeeklyRestDayRead]:
    stmt = select(EmployeeWeeklyRestDay).order_by(
        EmployeeWeeklyRestDay.employee_id.asc(),
        EmployeeWeeklyRestDay.weekday.asc(),
    )
    if employee_id is not None:
        stmt = stmt.where(EmployeeWeeklyRestDay.employee_id == employee_id)
    if active_only:
        stmt = stmt.where(EmployeeWeeklyRestDay.is_active.is_(True))
    return list(db.scalars(stmt).all())


@router.delete(
    "/admin/employee-weekly-rest-days/{rest_day_id}",
    response_model=SoftDeleteResponse,
    dependencies=[Depends(require_admin_permission("schedule", write=True))],
)
def deactivate_employee_weekly_rest_day(
    rest_day_id: int,
    request: Request,
    db: Session = Depends(get_db),
) -> SoftDeleteResponse:
    rest_day = db.get(EmployeeWeeklyRestDay, rest_day_id)
    if rest_day is None:
        raise HTTPException(status_code=404, detail="Employee weekly rest day not found")
    rest_day.is_active = False
    db.commit()
    log_audit(
        db,
        actor_type=AuditActorType.ADMIN,
        actor_id="admin",
        action="EMPLOYEE_WEEKLY_REST_DAY_DEACTIVATED",
        success=True,
        entity_type="employee_weekly_rest_day",
        entity_id=str(rest_day.id),
        ip=_client_ip(request),
        user_agent=_user_agent(request),
        details={"employee_id": rest_day.employee_id, "weekday": rest_day.weekday},
        request_id=getattr(request.state, "request_id", None),
    )
    return SoftDeleteResponse(ok=True, id=rest_day.id)


def _validate_special_day_scope(
    db: Session,
    *,
    department_id: int | None,
    region_id: int | None,
) -> tuple[int | None, int | None]:
    if department_id is not None and region_id is not None:
        raise HTTPException(status_code=422, detail="department_id and region_id cannot both be set")
...
@router.post(
    "/admin/special-days",
    response_model=SpecialDayRead,
    dependencies=[Depends(require_admin_permission("schedule", write=True))],
)
def upsert_special_day(
    payload: SpecialDayUpsertRequest,
    request: Request,
    db: Session = Depends(get_db),
) -> SpecialDayRead:
    department_id, region_id = _validate_special_day_scope(
        db,
        department_id=payload.department_id,
        region_id=payload.region_id,
    )
    if payload.id is None:
        special_day = SpecialDay(
            ...
            planned_minutes_override=payload.planned_minutes_override,
            half_day_overtime_start=payload.half_day_overtime_start,
            counts_as_paid_leave=payload.counts_as_paid_leave,
            overtime_code=payload.overtime_code,
        ...
        special_day = db.get(SpecialDay, payload.id)
        if special_day is None:
            raise HTTPException(status_code=404, detail="Special day not found")
        special_day.day_date = payload.day_date
        special_day.name = payload.name
        special_day.day_type = payload.day_type
        special_day.work_policy = payload.work_policy
        special_day.planned_minutes_override = payload.planned_minutes_override
        special_day.half_day_overtime_start = payload.half_day_overtime_start
        special_day.counts_as_paid_leave = payload.counts_as_paid_leave
        special_day.overtime_code = payload.overtime_code
        special_day.overtime_multiplier = payload.overtime_multiplier
        special_day.department_id = department_id
        special_day.region_id = region_id
        special_day.is_active = payload.is_active
        special_day.note = payload.note
    ...
    log_audit(
        ...
        actor_type=AuditActorType.ADMIN,
        actor_id="admin",
        action="SPECIAL_DAY_UPSERT",
        success=True,
        entity_type="special_day",
        entity_id=str(special_day.id),
        ip=_client_ip(request),
        user_agent=_user_agent(request),
        details={
            "day_date": special_day.day_date.isoformat(),
            "day_type": special_day.day_type.value,
            "work_policy": special_day.work_policy.value,
            "overtime_code": special_day.overtime_code.value,
            "department_id": special_day.department_id,
            "region_id": special_day.region_id,
            "is_active": special_day.is_active,
        },
        request_id=getattr(request.state, "request_id", None),
    )
    return special_day
...
@router.get(
    "/admin/special-days",
    response_model=list[SpecialDayRead],
    dependencies=[Depends(require_admin_permission("schedule"))],
)
def list_special_days(
    ...
    stmt = select(SpecialDay).order_by(SpecialDay.day_date.asc(), SpecialDay.id.asc())
    if start_date is not None:
        stmt = stmt.where(SpecialDay.day_date >= start_date)
    if end_date is not None:
        stmt = stmt.where(SpecialDay.day_date <= end_date)
    if department_id is not None:
        stmt = stmt.where(SpecialDay.department_id == department_id)
    if region_id is not None:
        stmt = stmt.where(SpecialDay.region_id == region_id)
    if active_only:
        stmt = stmt.where(SpecialDay.is_active.is_(True))
    return list(db.scalars(stmt).all())


@router.delete(
    "/admin/special-days/{special_day_id}",
    response_model=SoftDeleteResponse,
    dependencies=[Depends(require_admin_permission("schedule", write=True))],
)
def deactivate_special_day(
    special_day_id: int,
    request: Request,
    db: Session = Depends(get_db),
) -> SoftDeleteResponse:
    special_day = db.get(SpecialDay, special_day_id)
    if special_day is None:
        raise HTTPException(status_code=404, detail="Special day not found")
    ...
    log_audit(
        ...
        actor_type=AuditActorType.ADMIN,
        actor_id="admin",
        action="SPECIAL_DAY_DEACTIVATED",
        success=True,
        entity_type="special_day",
        entity_id=str(special_day.id),
        ip=_client_ip(request),
        user_agent=_user_agent(request),
        details={"day_date": special_day.day_date.isoformat()},
        request_id=getattr(request.state, "request_id", None),
    )
    return SoftDeleteResponse(ok=True, id=special_day.id)


def _validate_special_day_employee_override_scope(
    *,
    special_day: SpecialDay,
    employee: Employee,
) -> None:
    if special_day.department_id is not None and employee.department_id != special_day.department_id:
        raise HTTPException(status_code=422, detail="Employee is outside the special day department scope")
    if special_day.region_id is not None and employee.region_id != special_day.region_id:
        raise HTTPException(status_code=422, detail="Employee is outside the special day region scope")


@router.post(
    "/admin/special-day-employee-overrides",
    response_model=SpecialDayEmployeeOverrideRead,
    dependencies=[Depends(require_admin_permission("schedule", write=True))],
)
def upsert_special_day_employee_override(
    payload: SpecialDayEmployeeOverrideUpsertRequest,
    request: Request,
    db: Session = Depends(get_db),
) -> SpecialDayEmployeeOverrideRead:
    special_day = db.get(SpecialDay, payload.special_day_id)
    if special_day is None:
        raise HTTPException(status_code=404, detail="Special day not found")
    employee = db.get(Employee, payload.employee_id)
    if employee is None:
        raise HTTPException(status_code=404, detail="Employee not found")
    _validate_special_day_employee_override_scope(special_day=special_day, employee=employee)

    if payload.id is not None:
        override = db.get(SpecialDayEmployeeOverride, payload.id)
        if override is None:
            raise HTTPException(status_code=404, detail="Special day employee override not found")
    else:
        override = db.scalar(
            select(SpecialDayEmployeeOverride).where(
                SpecialDayEmployeeOverride.special_day_id == payload.special_day_id,
                SpecialDayEmployeeOverride.employee_id == payload.employee_id,
            )
        )
        if override is None:
            override = SpecialDayEmployeeOverride(
                special_day_id=payload.special_day_id,
                employee_id=payload.employee_id,
            )
            db.add(override)

    override.special_day_id = payload.special_day_id
    override.employee_id = payload.employee_id
    override.work_policy = payload.work_policy
    override.planned_minutes_override = payload.planned_minutes_override
    override.counts_as_paid_leave = payload.counts_as_paid_leave
    override.overtime_code = payload.overtime_code
    override.overtime_multiplier = payload.overtime_multiplier
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=409, detail="Employee override already exists for this special day") from exc
    db.refresh(override)
    log_audit(
        db,
        actor_type=AuditActorType.ADMIN,
        actor_id="admin",
        action="SPECIAL_DAY_EMPLOYEE_OVERRIDE_UPSERT",
        success=True,
        entity_type="special_day_employee_override",
        entity_id=str(override.id),
        ip=_client_ip(request),
        user_agent=_user_agent(request),
        details={
            "special_day_id": override.special_day_id,
            "employee_id": override.employee_id,
            "work_policy": override.work_policy.value,
            "overtime_code": override.overtime_code.value,
...
@router.get(
    "/admin/special-day-employee-overrides",
    response_model=list[SpecialDayEmployeeOverrideRead],
    dependencies=[Depends(require_admin_permission("schedule"))],
)
def list_special_day_employee_overrides(
    special_day_id: int | None = Query(default=None, ge=1),
    employee_id: int | None = Query(default=None, ge=1),
    active_only: bool = Query(default=True),
    db: Session = Depends(get_db),
) -> list[SpecialDayEmployeeOverrideRead]:
    stmt = select(SpecialDayEmployeeOverride).order_by(
        SpecialDayEmployeeOverride.special_day_id.asc(),
        SpecialDayEmployeeOverride.employee_id.asc(),
    )
    if special_day_id is not None:
        stmt = stmt.where(SpecialDayEmployeeOverride.special_day_id == special_day_id)
    if employee_id is not None:
        stmt = stmt.where(SpecialDayEmployeeOverride.employee_id == employee_id)
    if active_only:
        stmt = stmt.where(SpecialDayEmployeeOverride.is_active.is_(True))
    return list(db.scalars(stmt).all())


@router.delete(
    "/admin/special-day-employee-overrides/{override_id}",
    response_model=SoftDeleteResponse,
    dependencies=[Depends(require_admin_permission("schedule", write=True))],
)
def deactivate_special_day_employee_override(
    override_id: int,
    request: Request,
    db: Session = Depends(get_db),
) -> SoftDeleteResponse:
    override = db.get(SpecialDayEmployeeOverride, override_id)
    if override is None:
        raise HTTPException(status_code=404, detail="Special day employee override not found")
    ...
    log_audit(
        ...
        actor_type=AuditActorType.ADMIN,
        actor_id="admin",
        action="SPECIAL_DAY_EMPLOYEE_OVERRIDE_DEACTIVATED",
        success=True,
        entity_type="special_day_employee_override",
        entity_id=str(override.id),
        ip=_client_ip(request),
        user_agent=_user_agent(request),
        details={"special_day_id": override.special_day_id, "employee_id": override.employee_id},
        request_id=getattr(request.state, "request_id", None),
    )
    return SoftDeleteResponse(ok=True, id=override.id)
...
def _validate_schedule_plan_payload(
    ...
    if (
        ...
        and payload.overtime_threshold_minutes is None
    ):
        raise HTTPException(
            status_code=422,
...
def _normalize_employee_ids(raw_ids: list[int] | None) -> list[int]:
    normalized: list[int] = []
    seen: set[int] = set()
    for raw_id in raw_ids or []:
        employee_id = int(raw_id)
        if employee_id <= 0 or employee_id in seen:
            continue
...
def _saturday_rotation_to_read(rotation: SaturdayRotation) -> SaturdayRotationRead:
    days = sorted(rotation.days or [], key=lambda item: item.day_date)
    return SaturdayRotationRead(
        id=rotation.id,
        ...
        team_size=rotation.team_size,
        repeat_interval_weeks=rotation.repeat_interval_weeks,
        ...
        days=[
            {
                ...
                "employee_ids": [
                    ...
                    for assignment in sorted(day.employees or [], key=lambda item: item.sort_order)
                ],
                "schedule_plan_id": day.schedule_plan_id,
...
def _load_saturday_rotation(db: Session, rotation_id: int) -> SaturdayRotation | None:
    return db.scalar(
        select(SaturdayRotation)
        .options(
            selectinload(SaturdayRotation.days).selectinload(SaturdayRotationDay.employees)
        )
        .where(SaturdayRotation.id == rotation_id)
    )


def _validate_saturday_rotation_payload(
    db: Session,
    payload: SaturdayRotationUpsertRequest,
) -> tuple[Department, DepartmentShift, dict[int, Employee]]:
    if payload.start_date > payload.end_date:
        raise HTTPException(status_code=422, detail="start_date cannot be greater than end_date")
    ...
    seen_day_dates: set[date] = set()
    for day in payload.days:
        if day.day_date.weekday() != 5:
            raise HTTPException(status_code=422, detail="Rotation days must be Saturdays")
        if day.day_date < payload.start_date or day.day_date > payload.end_date:
            raise HTTPException(status_code=422, detail="Rotation day must be inside date range")
        if day.day_date in seen_day_dates:
            raise HTTPException(status_code=422, detail="Duplicate rotation day")
        seen_day_dates.add(day.day_date)
        employee_ids.extend(_normalize_employee_ids(day.employee_ids))

    employee_map: dict[int, Employee] = {}
    normalized_employee_ids = _normalize_employee_ids(employee_ids)
    if normalized_employee_ids:
        employees = list(
            db.scalars(select(Employee).where(Employee.id.in_(normalized_employee_ids))).all()
        )
        ...
        for employee_id in normalized_employee_ids:
            ...
            if employee.department_id != payload.department_id:
                raise HTTPException(
                    status_code=422,
                    detail="Rotation employees must belong to selected department",
                )

    return department, shift, employee_map


def _sync_schedule_plan_targets(plan: DepartmentSchedulePlan, employee_ids: list[int]) -> None:
    desired_ids = set(employee_ids)
    existing_rows = {item.employee_id: item for item in list(plan.target_employees or [])}

    for employee_id, row in existing_rows.items():
        if employee_id not in desired_ids:
            plan.target_employees.remove(row)

    for employee_id in employee_ids:
        if employee_id not in existing_rows:
            plan.target_employees.append(DepartmentSchedulePlanEmployee(employee_id=employee_id))

    plan.target_employee_id = employee_ids[0] if employee_ids else None


def _sync_saturday_rotation_day_plan(
    db: Session,
    *,
    rotation: SaturdayRotation,
    day: SaturdayRotationDay,
    employee_ids: list[int],
) -> None:
    should_have_plan = rotation.is_active and day.is_active and bool(employee_ids)
    plan = db.get(DepartmentSchedulePlan, day.schedule_plan_id) if day.schedule_plan_id else None
    ...
    note = f"Cumartesi rotasyonu #{rotation.id}: {rotation.name} / {day.day_date.isoformat()}"
    if plan is None:
        ...
        db.add(plan)
        db.flush()
        day.schedule_plan_id = plan.id
    else:
        plan.department_id = rotation.department_id
        plan.target_type = SchedulePlanTargetType.ONLY_EMPLOYEE
        plan.shift_id = rotation.shift_id
        plan.start_date = day.day_date
        plan.end_date = day.day_date
        plan.is_locked = True
        plan.is_active = True
        plan.note = note

    _sync_schedule_plan_targets(plan, employee_ids)
...
@router.post(
    "/api/admin/saturday-rotations",
    response_model=SaturdayRotationRead,
    dependencies=[Depends(require_admin_permission("schedule", write=True))],
)
def upsert_saturday_rotation(
    payload: SaturdayRotationUpsertRequest,
    request: Request,
    db: Session = Depends(get_db),
) -> SaturdayRotationRead:
    _validate_saturday_rotation_payload(db, payload)
    ...
    if payload.id is None:
        rotation = SaturdayRotation(
            ...
            repeat_interval_weeks=payload.repeat_interval_weeks,
            is_active=payload.is_active,
            note=payload.note,
        )
        db.add(rotation)
        ...
        rotation = _load_saturday_rotation(db, payload.id)
        if rotation is None:
            raise HTTPException(status_code=404, detail="Saturday rotation not found")
        rotation.department_id = payload.department_id
        rotation.shift_id = payload.shift_id
        rotation.name = payload.name.strip()
        rotation.start_date = payload.start_date
        rotation.end_date = payload.end_date
        rotation.team_size = payload.team_size
        rotation.repeat_interval_weeks = payload.repeat_interval_weeks
        rotation.is_active = payload.is_active
        rotation.note = payload.note

    existing_days = {day.day_date: day for day in list(rotation.days or [])}
    desired_dates = {day.day_date for day in payload.days}

    for day_date, existing_day in existing_days.items():
        if day_date not in desired_dates:
            existing_day.is_active = False
            _sync_saturday_rotation_day_plan(
                db,
                rotation=rotation,
                day=existing_day,
                employee_ids=[],
    ...
    for day_payload in sorted(payload.days, key=lambda item: item.day_date):
        employee_ids = _normalize_employee_ids(day_payload.employee_ids)
        rotation_day = existing_days.get(day_payload.day_date)
        if rotation_day is None:
            rotation_day = SaturdayRotationDay(
                rotation_id=rotation.id,
                day_date=day_payload.day_date,
                is_active=day_payload.is_active,
                note=day_payload.note,
            )
            rotation.days.append(rotation_day)
            db.flush()
        else:
            rotation_day.is_active = day_payload.is_active
            rotation_day.note = day_payload.note

        existing_assignments = {
            item.employee_id: item for item in list(rotation_day.employees or [])
        }
        desired_ids = set(employee_ids)
        for employee_id, assignment in existing_assignments.items():
            if employee_id not in desired_ids:
                rotation_day.employees.remove(assignment)
        for sort_order, employee_id in enumerate(employee_ids):
            assignment = existing_assignments.get(employee_id)
            if assignment is None:
                rotation_day.employees.append(
                    SaturdayRotationDayEmployee(
                        employee_id=employee_id,
                        sort_order=sort_order,
                    )
    ...
    db.commit()
    saved_rotation = _load_saturday_rotation(db, rotation.id)
    assert saved_rotation is not None

    log_audit(
        db,
        actor_type=AuditActorType.ADMIN,
        actor_id="admin",
        action="SATURDAY_ROTATION_UPSERT",
        success=True,
        entity_type="saturday_rotation",
        entity_id=str(saved_rotation.id),
        ip=_client_ip(request),
        user_agent=_user_agent(request),
        details={
            "department_id": saved_rotation.department_id,
            "shift_id": saved_rotation.shift_id,
            "start_date": saved_rotation.start_date.isoformat(),
            "end_date": saved_rotation.end_date.isoformat(),
            "day_count": len(saved_rotation.days or []),
            "is_active": saved_rotation.is_active,
        },
        request_id=getattr(request.state, "request_id", None),
    )
    return _saturday_rotation_to_read(saved_rotation)


@router.get(
    "/api/admin/saturday-rotations",
    response_model=list[SaturdayRotationRead],
    dependencies=[Depends(require_admin_permission("schedule"))],
)
def list_saturday_rotations(
    department_id: int | None = Query(default=None, ge=1),
    active_only: bool = Query(default=True),
    start_date: date | None = Query(default=None),
    end_date: date | None = Query(default=None),
    db: Session = Depends(get_db),
) -> list[SaturdayRotationRead]:
    stmt = (
        select(SaturdayRotation)
        .options(
            selectinload(SaturdayRotation.days).selectinload(SaturdayRotationDay.employees)
        )
        .order_by(SaturdayRotation.id.desc())
    )
    if department_id is not None:
        stmt = stmt.where(SaturdayRotation.department_id == department_id)
    if active_only:
        stmt = stmt.where(SaturdayRotation.is_active.is_(True))
    if start_date is not None:
        stmt = stmt.where(SaturdayRotation.end_date >= start_date)
    if end_date is not None:
        stmt = stmt.where(SaturdayRotation.start_date <= end_date)
    return [_saturday_rotation_to_read(item) for item in db.scalars(stmt).all()]


@router.get(
    "/api/admin/saturday-rotations/{rotation_id}",
    response_model=SaturdayRotationRead,
    dependencies=[Depends(require_admin_permission("schedule"))],
)
def get_saturday_rotation(
    rotation_id: int,
    db: Session = Depends(get_db),
) -> SaturdayRotationRead:
    rotation = _load_saturday_rotation(db, rotation_id)
    if rotation is None:
        raise HTTPException(status_code=404, detail="Saturday rotation not found")
    return _saturday_rotation_to_read(rotation)
...
def deactivate_saturday_rotation(
    rotation_id: int,
    request: Request,
    db: Session = Depends(get_db),
) -> SoftDeleteResponse:
    ...
    for day in rotation.days or []:
        day.is_active = False
        _sync_saturday_rotation_day_plan(db, rotation=rotation, day=day, employee_ids=[])
    db.commit()
    log_audit(
        db,
        actor_type=AuditActorType.ADMIN,
        actor_id="admin",
        action="SATURDAY_ROTATION_DEACTIVATED",
        success=True,
        entity_type="saturday_rotation",
        entity_id=str(rotation.id),
        ip=_client_ip(request),
        user_agent=_user_agent(request),
        details={"department_id": rotation.department_id},
        request_id=getattr(request.state, "request_id", None),
    )
    return SoftDeleteResponse(ok=True, id=rotation.id)
...
def upsert_schedule_plan(
    ...
    if payload.id is None:
        ...
        plan.overtime_threshold_minutes = payload.overtime_threshold_minutes
...
@router.get(
    "/api/admin/leaves/ledger",
    response_model=LeaveLedgerResponse,
    dependencies=[Depends(require_admin_permission("leaves"))],
)
def leave_ledger_endpoint(
    as_of: date | None = Query(default=None),
    year: int | None = Query(default=None, ge=1970, le=2100),
    db: Session = Depends(get_db),
) -> LeaveLedgerResponse:
    reference = as_of or date.today()
    rows = build_leave_ledger(db, as_of=reference, year=year)
    return LeaveLedgerResponse(as_of=reference, year=year, rows=rows)


@router.get(
    "/api/admin/leaves/ledger.xlsx",
    dependencies=[Depends(require_admin_permission("leaves"))],
)
def leave_ledger_xlsx_endpoint(
    as_of: date | None = Query(default=None),
    year: int | None = Query(default=None, ge=1970, le=2100),
    db: Session = Depends(get_db),
) -> Response:
    reference = as_of or date.today()
    rows = build_leave_ledger(db, as_of=reference, year=year)
    payload = build_leave_ledger_xlsx_bytes(rows, as_of=reference, year=year, generated_at=datetime.now(timezone.utc))
    return Response(
        content=payload,
        media_type=XLSX_MEDIA_TYPE,
        headers={"Content-Disposition": 'attachment; filename="yillik-izin-defteri.xlsx"'},
    )
...
@router.get(
    ...
    dependencies=[Depends(require_admin_permission("communications"))],
)
...
def admin_conversation_create_endpoint(
    payload: AdminConversationCreateRequest,
    request: Request,
    claims: dict[str, Any] = Depends(require_admin_permission("communications", write=True)),
    db: Session = Depends(get_db),
) -> EmployeeConversationThreadRead:
    actor_id, actor_admin_user_id = _normalized_admin_actor_from_claims(claims)
    ...
    log_audit(
        ...
        actor_type=AuditActorType.ADMIN,
        actor_id=actor_id,
        action="ADMIN_CONVERSATION_CREATED",
        success=True,
...
def list_notification_delivery_logs(
    ...
    admin_subscription_by_endpoint: dict[str, AdminPushSubscription] = {}
    admin_log_endpoints = {
        row.endpoint
        for row in rows
        if row.recipient_type == "admin" and isinstance(row.endpoint, str) and row.endpoint
    }
    if admin_log_endpoints:
        admin_subscription_by_endpoint = {
            row.endpoint: row
            for row in db.scalars(
                select(AdminPushSubscription).where(
                    AdminPushSubscription.endpoint.in_(admin_log_endpoints)
                )
            ).all()
        }
    ...
    for row in rows:
        ...
        if row.recipient_type == "employee":
            ...
            elif row.recipient_address:
                recipient_name = row.recipient_address
            elif row.endpoint:
                admin_subscription = admin_subscription_by_endpoint.get(row.endpoint)
                if admin_subscription is not None:
                    recipient_id = admin_subscription.admin_user_id
                    recipient_name = admin_subscription.admin_username
...
@router.get(
    "/api/admin/attendance/daily-board/export.xlsx",
    dependencies=[Depends(require_admin_permission("reports"))],
)
def export_attendance_daily_board(
    request: Request,
    ...
    payload = build_daily_board_xlsx_bytes(
        db,
        target_date=target_date,
    ...
    filename = f"calisan-durum-{target_date.isoformat()}.xlsx"
    return Response(
        content=payload,
        media_type=XLSX_MEDIA_TYPE,
        headers={"Content-Disposition": f'attachment; filename="{filename}"'},

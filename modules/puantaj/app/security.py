...
def verify_admin_credentials(username: str, password: str) -> bool:
    ...
    # A non-bcrypt value means ADMIN_PASS_HASH is misconfigured; never fall back to
    # plaintext comparison, that would let the env file hold the real password.
    if not env_pass_hash.startswith(("$2a$", "$2b$", "$2y$")):
        return False
...
def _refresh_claims_from_db(claims: dict[str, Any], db: Session) -> dict[str, Any]:
    """Overlay claims with the admin user's *current* DB state.

    Admin access tokens can legitimately live for a long time (see
    ``Settings.access_token_minutes``). If an admin's permissions, active
    status, or super-admin flag are changed after the token was issued, the
    change must take effect immediately for permission checks rather than
    waiting for the token to expire and be refreshed (or for the user to log
    out/in manually). Only ``admin_users`` table rows are looked up here; the
    bootstrap/env admin (no ``admin_user_id``) keeps its token claims as-is
    since it has no DB-backed permission record.
    """
    admin_user_id = claims.get("admin_user_id")
    if not admin_user_id:
        return claims

    # Import locally to avoid a hard import-time dependency between
    # app.security and app.models for modules that only need token utilities.
    ...
    refreshed["is_super_admin"] = bool(admin_user.is_super_admin)
    refreshed["permissions"] = admin_user.permissions
    return refreshed
...
def require_admin_permission(permission: str, *, write: bool = False) -> Callable[..., dict[str, Any]]:
    ...
    def _dependency(
        ...
        live_claims = _refresh_claims_from_db(claims, db)
        if not has_permission(live_claims, permission, write=write):
            raise ApiError(status_code=403, code="FORBIDDEN", message="Insufficient permissions.")
        return live_claims
...
def require_admin_any_permission(*permissions: str, write: bool = False) -> Callable[..., dict[str, Any]]:
    ...
    def _dependency(
        ...
        live_claims = _refresh_claims_from_db(claims, db)
        if not any(has_permission(live_claims, permission, write=write) for permission in permissions):
            raise ApiError(status_code=403, code="FORBIDDEN", message="Insufficient permissions.")
        return live_claims

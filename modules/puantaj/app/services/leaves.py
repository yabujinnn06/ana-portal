...
# --- Yillik izin defteri (4857 sayili Is Kanunu md.53) ---


def annual_entitlement_for_service_year(n: int) -> int:
    """n. hizmet yilini dolduran calisanin o yil icin hak ettigi yillik izin gunu."""
    if n <= 0:
        return 0
    if n <= 5:  # 1-5 yil (dahil)
        return 14
    if n < 15:  # 5 ustu, 15 alti
        return 20
    return 26  # 15 yil ve fazlasi


def cumulative_annual_entitlement(completed_years: int) -> int:
    """Tamamlanan hizmet yillari boyunca toplam hak edilen yillik izin (kumulatif)."""
    return sum(annual_entitlement_for_service_year(n) for n in range(1, max(0, completed_years) + 1))
...
def seniority_label(hire_date: date, as_of: date) -> str:
    if as_of < hire_date:
        return "0 ay"
    years = as_of.year - hire_date.year
    ...
    if months == 0:
        return f"{years} yil"
    return f"{years} yil {months} ay"


def annual_leave_used_days(start_date: date, end_date: date, half_day: bool) -> Decimal:
    """Yillik izin gun sayisi (is gunu; Pazar haric). Tek gun + half_day = 0,5."""
    if half_day and start_date == end_date:
        return Decimal("0.5")
    count = 0
    cursor = start_date
    while cursor <= end_date:
        if cursor.weekday() != 6:  # Pazar haric (hafta tatili)
            count += 1
        cursor += timedelta(days=1)
    return Decimal(count)


def build_leave_ledger(db: Session, *, as_of: date, year: int | None = None) -> list[dict]:
    """Aktif calisanlar icin yillik izin defteri: kidem, hak edilen, kullanilan, kalan."""
    employees = list(
        db.scalars(
    ...
    profiles = {
        p.employee_id: p
        for p in db.scalars(select(EmployeePayrollProfile)).all()
    }
    leaves = list(
        db.scalars(
            select(Leave).where(
    ...
    leaves_by_employee: dict[int, list[Leave]] = {}
    for leave in leaves:
    ...
    for employee in employees:
        ...
        used_total = sum(
            (annual_leave_used_days(l.start_date, l.end_date, l.half_day) for l in emp_leaves if l.start_date <= as_of),
            Decimal("0"),
        )
        used_year = (
            sum(
                (
                    annual_leave_used_days(l.start_date, l.end_date, l.half_day)
                    for l in emp_leaves
        ...
        if hire_date is not None and hire_date <= as_of:
            completed = completed_service_years(hire_date, as_of)
            entitled_total = Decimal(cumulative_annual_entitlement(completed))
            current_rate = annual_entitlement_for_service_year(completed + 1)
            seniority = seniority_label(hire_date, as_of)
        else:
            completed = 0
            entitled_total = Decimal("0")
            current_rate = 0
            seniority = "-"
        rows.append(
            {
                ...
                "hire_date": hire_date,
                "has_hire_date": hire_date is not None,
                ...
                "used_year": used_year,
                "remaining": entitled_total - used_total,
            }
        )
    return rows

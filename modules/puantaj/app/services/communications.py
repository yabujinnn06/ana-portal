...
def _notify_employee_about_new_conversation(
    db: Session,
    *,
    conversation: EmployeeConversation,
    employee: Employee,
    ...
    try:
        send_push_to_employees(
            ...
            body=f"{conversation.subject}: {message[:120]}",
            data={
                "type": "EMPLOYEE_CONVERSATION_ADMIN_STARTED",
                "conversation_id": conversation.id,
                "employee_id": employee.id,
                "url": f"/employee/?conversation_id={conversation.id}&communication=1",
            },
        ...
        logger.exception(
            "employee_conversation_admin_started_push_failed",
            extra={"conversation_id": conversation.id, "employee_id": employee.id},
        )
...
def create_admin_conversation(
    ...
    *,
    ...
    if not employee.is_active:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Inactive employee cannot receive messages.",
        )
    ...
    conversation = _conversation_with_thread_or_404(db, conversation_id=conversation.id)
    _notify_employee_about_new_conversation(
        db,
        conversation=conversation,
        employee=employee,
        message=payload.message,

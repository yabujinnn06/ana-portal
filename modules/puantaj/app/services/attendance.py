...
from app.services.location import distance_m, evaluate_location, should_block_attendance
...
QR_ACCURACY_TOLERANCE_CAP_M = 100.0
...
def create_employee_qr_scan_event(
    ...
    *,
    ...
    accuracy_tolerance_m = min(max(accuracy_m or 0.0, 0.0), QR_ACCURACY_TOLERANCE_CAP_M)
    ...
    for point in active_points:
        ...
        if point_distance_m <= float(point.radius_m) + accuracy_tolerance_m:
...
def _build_attendance_event(
    ...
    *,
    ...
    if should_block_attendance(location_status, employee_location):
        raise ApiError(
            status_code=403,
            code="LOCATION_REJECTED",
            message="Konum dogrulanamadi. Mesai noktasinda olduğunuzdan emin olun veya yoneticinize basvurun.",
        )
...
def get_employee_status_by_device(
    ...
    *,
    ...
    # A night shift that started before midnight stays open into the next local day.
    # _resolve_active_open_shift_event looks back one day for midnight-crossing shifts,
    # matching what check-in/check-out enforcement uses. The today-only summary above
    # misses it, so reflect it here; otherwise the employee is told NOT_STARTED/CHECKIN
    # while check-in is actually blocked and check-out stays hidden.
    active_open_event = _resolve_active_open_shift_event(
        db,
        employee=employee,
        reference_ts_utc=reference_now_utc,
    ...
    if (
        ...
        and _local_day_from_utc(active_open_event.ts_utc) != _local_day_from_utc(reference_now_utc)
    ):
        has_open_shift = True
        today_status = "IN_PROGRESS"
        last_in_event = active_open_event
        if last_event_today is None:
            last_event_today = active_open_event

...
# Calisana fazla mesai oldugunu caktirmadan, her fazla mesai saati basinda
# gonderilen notr "cikis yapmayi unutma" hatirlatmasi.
TYPE_OVERTIME_CHECKOUT_REMINDER = "cikis_hatirlatma_saatlik"

# Employee cihazlarinda sadece explicit onay, demo akislari ve saatlik cikis
# hatirlatmasi gorunsun.
...
class DayAssessment:
    ...
    def shift_window_local(self) -> str:
        if self.shift is None:
            return f"Tanimli vardiya yok (planli sure: {_format_minutes_label(self.planned_minutes)})"
...
def _resolve_shift_for_day(
    ...
    *,
    ...
    first_checkin_shift: DepartmentShift | None = None
    if first_checkin_event is not None:
        shift_id = _extract_shift_id_from_flags(first_checkin_event.flags)
        if shift_id is not None:
            first_checkin_shift = session.get(DepartmentShift, shift_id)
    ...
    if first_checkin_shift is not None:
        effective_shift = first_checkin_shift
    if effective_shift is None and plan is not None and plan.shift_id is not None:
...
def _monitor_payload(
    *,
    ...
    payload: dict[str, Any] = {
        ...
        "shift_window_is_inferred": assessment.shift is None,
        "planned_minutes": assessment.planned_minutes,
...
def _create_notification_job(
    ...
    *,
    ...
    event_variant = variant_override or (
...
def _schedule_overtime_checkout_reminder(
    session: Session,
    *,
    created_jobs: list[NotificationJob],
    assessment: DayAssessment,
    ...
    # Calisana fazla mesai oldugunu caktirmadan, fazla mesainin her tam saati
    # basinda employee cihazina sert (in-app + telefon) cikis hatirlatmasi yollar.
    if assessment.first_checkin_ts_utc is None or assessment.checkout_ts_utc is not None:
        return
    overtime_start_utc = _overtime_start_utc(assessment)
    elapsed_hours = int((now_utc - overtime_start_utc).total_seconds() // 3600)
    if elapsed_hours < 1:
        return
    # Her saat icin tek bildirim (variant ile dedup). 6 saatte otomatik kapatma
    # devreye girdigi icin ust sinir OVERTIME_MAX_HOURS ile sinirli.
    hour_bucket = min(elapsed_hours, OVERTIME_MAX_HOURS)
    boundary_utc = overtime_start_utc + timedelta(hours=hour_bucket)
    job = _create_notification_job(
        session,
        assessment=assessment,
        notification_type=TYPE_OVERTIME_CHECKOUT_REMINDER,
        audience=AUDIENCE_EMPLOYEE,
        risk_level=RISK_CRITICAL,
        event_ts_utc=boundary_utc,
        scheduled_at_utc=boundary_utc,
        title="Mesai hatirlatmasi",
        description="Mesainizi bitirmeyi unutmayin.",
        actual_time_summary="Mesai cikis kaydiniz hala acik gorunuyor.",
        suggested_action="Mesainiz bittiyse uygulamadan mesainizi bitirin.",
        variant_override=f"checkout-h{hour_bucket}",
        extra_payload={
            "employee_ids": [assessment.employee.id],
            "overtime_hour_bucket": hour_bucket,
        },
    )
    if job is not None:
        created_jobs.append(job)
    ...
    for employee in employees:
        ...
        for local_day in candidate_days:
            ...
            _schedule_overtime_checkout_reminder(
                session, created_jobs=created_jobs, assessment=assessment, now_utc=reference_utc
            )

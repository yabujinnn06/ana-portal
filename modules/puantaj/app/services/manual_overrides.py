...
from app.models import DepartmentShift, Employee, ManualDayOverride, ManualDayStatus
...
def upsert_manual_day_override(
    ...
    *,
    ...
    if payload.status is not None:
        day_status = ManualDayStatus(payload.status)
    elif payload.is_absent:
        # Legacy clients only send is_absent; keep their semantics.
        day_status = ManualDayStatus.CALISMADI
    else:
        day_status = ManualDayStatus.NORMAL
    is_absent = day_status is not ManualDayStatus.NORMAL
    ...
    if is_absent:
        ...
        if in_ts is not None and out_ts is not None and out_ts < in_ts:
            # Out before in means the shift crosses midnight (e.g. 22:00 -> 06:00);
            # the checkout actually lands on the next local day. Recombine on day+1 so
            # the timestamp is DST-correct rather than a raw +24h offset.
            out_ts = _combine_utc(payload.day_date + timedelta(days=1), payload.out_time)
    ...
    if override is None:
        ...
        override.is_absent = is_absent
        override.status = day_status

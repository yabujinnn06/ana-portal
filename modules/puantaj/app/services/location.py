...
from app.settings import get_location_enforcement_block_statuses, get_settings
...
def should_block_attendance(
    location_status: LocationStatus,
    employee_location: EmployeeLocation | None,
) -> bool:
    """Return True only when enforcement mode is 'strict' and the status is in the
    configured block list. Bypasses when the employee has no home location set
    (NOT_CONFIGURED state) to avoid locking out staff during rollout."""

    mode = (get_settings().location_enforcement_mode or "").strip().lower()
    if mode != "strict":
        return False
    if employee_location is None:
        return False
    block_statuses = get_location_enforcement_block_statuses()
    if not block_statuses:
        return False
    return location_status.value in block_statuses

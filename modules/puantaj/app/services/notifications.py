...
def _build_message_for_job(session: Session, job: NotificationJob) -> NotificationMessage:
    ...
    if job.job_type == JOB_TYPE_ATTENDANCE_MONITOR or job.notification_type:
        ...
        if audience == AUDIENCE_EMPLOYEE:
            # Calisana giden bildirim notr ve kisa olmali; admin formatindaki
            # vardiya/risk/fazla mesai detaylari calisana sizdirilmaz.
            return NotificationMessage(
                recipients=recipients,
                subject=title,
                body=description,
    ...
    *,
    ...
    if isinstance(push_deliveries, list):
        for row in push_deliveries:
            ...
            recipient_type = audience or (
                AUDIENCE_ADMIN
                if row.get("admin_user_id") is not None
                or (isinstance(admin_username, str) and admin_username.strip())
                else AUDIENCE_EMPLOYEE
            )
            status = str(row.get("status") or "FAILED").upper()
            if status not in {"SENT", "FAILED"}:
                status = "FAILED"
            admin_recipient_address = (
                admin_username.strip()
                if recipient_type == AUDIENCE_ADMIN
                and isinstance(admin_username, str)
            ...
            session.add(
                NotificationDeliveryLog(
                    ...
                    recipient_address=admin_recipient_address,

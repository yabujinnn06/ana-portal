...
def gross_minutes_between(first_in_ts: datetime | None, last_out_ts: datetime | None) -> int:
    """Brut giris-cikis araligi (mola dusulmez). FM2 tam gun / FM3 icin temel."""
    if first_in_ts is None or last_out_ts is None:
        return 0
    return max(0, int((last_out_ts - first_in_ts).total_seconds() // 60))


def minutes_after_shift_end(shift_end_ts: datetime | None, last_out_ts: datetime | None) -> int:
    """Vardiya bitisinden sonraki gercek sure (FM1). Mola/gec giris bunu azaltmaz."""
    if shift_end_ts is None or last_out_ts is None:
        return 0
    return max(0, int((last_out_ts - shift_end_ts).total_seconds() // 60))
...
def calculate_day_metrics(
    *,
    ...
    late_entry_tolerance_minutes: int = 0,
    ...
    # Overtime is measured from the shift start, not the raw clock-in, so an early
    # arrival never inflates overtime (the pre-shift minutes are simply ignored).
    # A late arrival within the configured tolerance is forgiven (anchor stays at the
    # shift start); once the tolerance is exceeded the whole lateness counts against
    # overtime (anchor moves to the real clock-in). Penalty mode: no partial credit.
    overtime_anchor_ts = first_in_ts
    if shift_start_ts is not None:
        late_minutes = max(0, int((first_in_ts - shift_start_ts).total_seconds() // 60))
        if late_minutes <= max(0, late_entry_tolerance_minutes):
            overtime_anchor_ts = shift_start_ts
    overtime_gross_minutes = max(0, int((last_out_ts - overtime_anchor_ts).total_seconds() // 60))
    overtime_worked_minutes = max(0, overtime_gross_minutes - effective_break)
    # NET hedef tanimliysa fazla mesai sabit esige gore olculur (mola net isten
    # dusuldugu icin FM'yi azaltir). Tanimsizsa eski davranis: gunluk plan esigi.
    overtime_threshold = (
        max(0, planned_minutes)
        if overtime_threshold_minutes is None
        else max(0, int(overtime_threshold_minutes))
    )
    overtime_minutes = max(
        0,
        overtime_worked_minutes - overtime_threshold - max(0, overtime_grace_minutes),
    )

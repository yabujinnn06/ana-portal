...
class _FailingRefreshPersistLoginDB(_FakeLoginDB):
    def __init__(self, admin_user: AdminUser):
        super().__init__(admin_user)
        self.fail_next_refresh_commit = True

    def commit(self) -> None:
        if self.fail_next_refresh_commit and any(isinstance(row, AdminRefreshToken) for row in self.rows):
            self.fail_next_refresh_commit = False
            raise RuntimeError("refresh token write failed")
        return

    def rollback(self) -> None:
        self.rows = [row for row in self.rows if not isinstance(row, AdminRefreshToken)]
...
class _FakeAdminUsersDB:
    # id=1 represents the DB-backed super admin actor whose token claims
    # (see tests below) carry ``admin_user_id: 1``; permission checks now
    # re-resolve the acting admin from the DB on every request.
    ACTOR_SUPER_ADMIN = AdminUser(
        id=1,
        username="admin",
        password_hash="",
    ...
    def get(self, model, pk):  # type: ignore[no-untyped-def]
        if model is AdminUser and pk == self.admin_user.id:
            return self.admin_user
        if model is AdminUser and pk == self.ACTOR_SUPER_ADMIN.id:
...
class _FakeAdminUsersListDB:
    ACTOR_SUPER_ADMIN = _FakeAdminUsersDB.ACTOR_SUPER_ADMIN
...
class AdminRbacTests(unittest.TestCase):
    ...
    def test_login_accepts_db_admin_user(self) -> None:
        ...
        self.assertEqual(payload.get("user", {}).get("username"), "ik_test")
    ...
    def test_login_still_succeeds_when_refresh_token_persist_fails(self) -> None:
        admin_user = AdminUser(
            id=14,
            username="ik_refresh_soft_fail",
            password_hash=hash_password("StrongPass123!"),
            full_name="IK Refresh Soft Fail",
            is_active=True,
            is_super_admin=False,
            permissions={"reports": {"read": True, "write": False}},
        )
        fake_db = _FailingRefreshPersistLoginDB(admin_user)
        app.dependency_overrides[get_db] = _override_get_db(fake_db)
        client = TestClient(app)

        response = client.post(
            "/api/admin/auth/login",
            json={"username": "ik_refresh_soft_fail", "password": "StrongPass123!"},
        )

        self.assertEqual(response.status_code, 200)
        payload = response.json()
        self.assertIn("access_token", payload)
        self.assertIsNone(payload.get("refresh_token"))
        self.assertEqual(payload.get("user", {}).get("username"), "ik_refresh_soft_fail")

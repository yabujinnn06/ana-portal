...
class InactiveEmployeeAccessTests(unittest.TestCase):
    ...
    def test_device_claim_is_idempotent_for_already_claimed_same_device(self) -> None:
        invite = DeviceInvite(
            id=101,
            employee_id=20,
            token="used-token",
        ...
        self.assertEqual(body["employee_id"], 20)
        self.assertEqual(body["device_id"], 43)

    def test_device_claim_reports_used_invite_when_device_is_not_same_claim(self) -> None:
        invite = DeviceInvite(
            id=102,
            employee_id=20,
            token="used-token-other-device",
            expires_at=datetime.now(timezone.utc) + timedelta(minutes=30),
            is_used=True,
        )

        fake_db = _FakeDB([invite, None])
        app.dependency_overrides[get_db] = _override_get_db(fake_db)
        client = TestClient(app)
        ...
        response = client.post(
            ...
            json={
                "token": "used-token-other-device",
                "device_fingerprint": "other-device-fp",
            },
        )

        self.assertEqual(response.status_code, 409)
        body = response.json()
        self.assertEqual(body["error"]["code"], "INVITE_ALREADY_USED")

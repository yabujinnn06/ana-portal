...
class _FakeEmployeeListDB:
    ...
    def execute(self, statement):  # type: ignore[no-untyped-def]
        stmt_text = str(statement)
        rows = self.active_rows if "employees.is_active IS true" in stmt_text else self.all_rows
        return _ScalarRows([(employee, 0, 0) for employee in rows])
...
class AdminPuantajFeatureTests(unittest.TestCase):
    ...
    def test_xlsx_export_formats_dashboard_and_duration_columns(self) -> None:
        ...
        header_map = {
            daily_sheet.cell(row=7, column=col_idx).value: col_idx
            for col_idx in range(1, daily_sheet.max_column + 1)
        }
        for name in ("Çalışma Süresi", "Mola", "Net Süre", "Erken Gelis", "Plan Üstü Süre"):
            self.assertEqual(
                daily_sheet.cell(row=day_row, column=header_map[name]).number_format,
                "[h]:mm",
                name,
            )
        for name in ("Net Süre (dk)", "Erken Gelis (dk)", "Plan Üstü Süre (dk)"):
            self.assertNotEqual(
                daily_sheet.cell(row=day_row, column=header_map[name]).number_format,
                "[h]:mm",
                name,
    ...
    def test_xlsx_export_all_mode_uses_clean_turkish_labels(self) -> None:
        ...
        self.assertEqual(daily_sheet["N7"].value, "Yasal Fazla Mesai")
        self.assertEqual(daily_sheet["X7"].value, "Gün Tipi")
        self.assertEqual(daily_sheet["Y7"].value, "Çalışıldı mı")
    ...
    def test_manual_override_changes_monthly_day_output(self) -> None:
        ...
        # 2026-02-01 Pazar: plan/mola uygulanmaz, calisma brut giris-cikis (09:30-18:30 = 540).
        self.assertEqual(first_day.worked_minutes, 540)
        self.assertEqual(first_day.fm3_minutes, 540)

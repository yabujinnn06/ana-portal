...
from app.models import AttendanceEvent, AttendanceType, Device, Employee, LocationStatus
...
class _DummyDB:
    ...
    @property
    def added_attendance_events(self) -> list[object]:
        return [obj for obj in self.added if isinstance(obj, AttendanceEvent)]
...
class AttendanceSequenceTests(unittest.TestCase):
    ...
    def test_checkin_is_allowed_when_daily_limit_not_reached(self) -> None:
        db = _DummyDB()
        device = _build_device()
        finished_last_event = SimpleNamespace(type=AttendanceType.OUT, lat=None, lon=None, ts_utc=None)
        ...
        self.assertEqual(len(db.added_attendance_events), 1)
    ...
    def test_checkout_is_allowed_after_midnight_when_open_shift_exists(self) -> None:
        db = _DummyDB()
        device = _build_device()
        open_last_event = SimpleNamespace(type=AttendanceType.IN, flags={}, lat=None, lon=None, ts_utc=None)
    ...
    def test_previous_day_stale_checkin_does_not_block_new_checkin(self) -> None:
        db = _DummyDB()
        device = _build_device()
        stale_last_event = SimpleNamespace(type=AttendanceType.IN, lat=None, lon=None, ts_utc=None)

...
class WeekdayShiftAssignmentsTests(unittest.TestCase):
    ...
    def test_infer_shift_from_checkin_time_chooses_closest_shift_start(self) -> None:
        employee = Employee(id=9, full_name="Basak Celik", department_id=11, shift_id=401, is_active=True)
        ...
        with patch(
            ...
            selected_shift, diff_minutes = _infer_shift_from_checkin_time(
                _DummyDB(),
                employee=employee,
                checkin_ts_utc=datetime(2026, 3, 2, 11, 37, tzinfo=timezone.utc),  # 14:37 local
            )

        self.assertIsNotNone(selected_shift)
        assert selected_shift is not None
        self.assertEqual(selected_shift.id, 402)
        self.assertEqual(diff_minutes, 37)

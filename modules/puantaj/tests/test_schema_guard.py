...
from app.services.schema_guard import schema_issues_all_transient, verify_runtime_schema
...
class SchemaIssuesTransientTests(unittest.TestCase):
    def test_all_operational_errors_are_transient(self) -> None:
        issues = [
            "TABLE_UNREADABLE:employees:OperationalError",
            "TABLE_UNREADABLE:device_invites:OperationalError",
        ]
        self.assertTrue(schema_issues_all_transient(issues))

    def test_missing_columns_is_not_transient(self) -> None:
        issues = [
            "TABLE_UNREADABLE:employees:OperationalError",
            "MISSING_COLUMNS:employees:shift_id",
        ]
        self.assertFalse(schema_issues_all_transient(issues))

    def test_missing_table_not_transient(self) -> None:
        # Tablo gercekten yoksa NoSuchTableError gelir; bu kalici, tekrar denenmemeli.
        self.assertFalse(schema_issues_all_transient(["TABLE_UNREADABLE:employees:NoSuchTableError"]))

    def test_empty_issues_not_transient(self) -> None:
        self.assertFalse(schema_issues_all_transient([]))

...
class NotificationServiceTests(unittest.TestCase):
    def test_record_delivery_logs_keeps_admin_username_for_push_rows(self) -> None:
        job = NotificationJob(
            id=1701,
            employee_id=None,
            admin_user_id=None,
            job_type="CUSTOM_ADMIN_PUSH",
            notification_type="custom_admin",
            audience=None,
            payload={},
            scheduled_at_utc=datetime(2026, 4, 1, 12, 0, tzinfo=timezone.utc),
            status="PENDING",
            attempts=0,
            idempotency_key="CUSTOM_ADMIN_PUSH:1701",
        )
        fake_session = _FakeDeliveryLogSession()
        ...
        self.assertIsInstance(log, NotificationDeliveryLog)
        self.assertEqual(log.recipient_type, "admin")
        self.assertIsNone(log.admin_user_id)
        self.assertEqual(log.recipient_address, "esra")
        self.assertEqual(log.endpoint, "https://push.example/endpoint")
